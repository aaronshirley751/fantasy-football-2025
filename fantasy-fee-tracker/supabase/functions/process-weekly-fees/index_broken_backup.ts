...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    // CRITICAL OPTIMIZATION: Execute independent operations in parallel
    console.log(`[${Date.now() - startTime}ms] Starting parallel data operations...`)
    ...
    console.log(`[${Date.now() - startTime}ms] Parallel data operations completed`)
    ...
    console.log(`[${Date.now() - startTime}ms] Fee processing completed`)
    
    // Get season summaries for Discord
    const seasonSummaries = await getSeasonSummaries(supabase, league.id, userMappings)
    
    console.log(`[${Date.now() - startTime}ms] Season summaries completed`)
    ...
    console.log(`[${totalTime}ms] TOTAL EXECUTION TIME - OPTIMIZATION SUCCESS!`)
    ...
    const response = {
      ...
      optimization_applied: 'parallel_data_fetching',
